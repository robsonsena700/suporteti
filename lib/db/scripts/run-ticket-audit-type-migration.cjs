...
async function main() {
  ...
  const fallback =
    "DO $$BEGIN IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid WHERE t.typname='ticket_audit_type' AND e.enumlabel='TICKET_UPDATED') THEN ALTER TYPE ticket_audit_type ADD VALUE 'TICKET_UPDATED'; END IF; END$$;";
